"""State abstraction encoders: raw, engineered, IB, VQ, contrastive."""
...
def engineered_aggregate(obs: np.ndarray) -> np.ndarray:
    ...
    feats = np.array(
        [
            local[0],
            local[2] if o.size > 2 else 0.0,
            local[3] if o.size > 3 else 0.0,
            local[7] if o.size > 7 else 0.0,
...
class ContrastiveEncoder(nn.Module):
    def __init__(self, obs_dim: int, latent_dim: int = 8, hidden: int = 64):
        ...
        self.proj = nn.Sequential(
            nn.Linear(latent_dim, hidden), nn.ReLU(), nn.Linear(hidden, latent_dim)
        )
...
class InformationBottleneckEncoder(IBEncoder):
    pass
